...
-(void)find_draw:(PDFVCanvas *)canvas :(PDFVPage *)page :(int)scrollx :(int) scrolly//draw current found
{
    if( !is_cancel )
    {
        [m_eve wait];
    ...
    if( m_finder != NULL && m_page_find_index >= 0 && m_page_find_index < m_page_find_cnt )
    {
        ...
        while( ichar < ichar_end )
        {
            ...
            else
            {
                ...
                [canvas FillRect:CGRectMake(rect_draw.left, rect_draw.top, (rect_draw.right - rect_draw.left), (rect_draw.bottom - rect_draw.top)): selColor];
                rect_word = rect;
            }
            ichar++;
        }
